...
import { getPendingBets, getUserCreatedEvents } from '../../redux/userReducer';
...
export default function Pendingbet() {

    const { loading, userInfo, pendingBets } = useSelector((state) => state.user);
    ...
    useEffect(() => {
        dispatch(getPendingBets({ token, condition: "pending" }));
    }, []);
    ...
    const columns = [
        {
            ...
            renderCell: (params) => {
                const index =
                pendingBets.findIndex((item) => item.id === params.row.id) + 1;
                return (
        ...
        //         return (
        //             <div onClick={() => setData(params.row)} onMouseOver={() => setHover(id)} onMouseOut={() => setHover("")} >
        //                 <svg width="38" height="37" viewBox="0 0 38 37" fill="none" xmlns="http://www.w3.org/2000/svg">
        //                     <rect x="0.652832" width="36.8377" height="36.8377" rx="18.4188" fill={hover !== id ? "#D2D2D2" : "#3FABDA"} />
        ...
        <section className="h-[100vh] lg:px-10 p-4 w-[100%] overflow-y-auto">
            ...
            <div className=' w-full py-6 flex flex-col gap-4 ' >
                <div className=' flex gap-3 ' >
                    ...
                    <p className=' text-lg font-bold text-black' >Pending Bet</p>
                </div>
            ...
            {Object.keys(data) && Object.keys(data).length > 1 && (
                <PlaceBet data={data} setData={() => setData({})} />
            )}{" "}
        </section>
    )
}
